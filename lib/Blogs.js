...
class Blogs {
	...
	getAllBlogs() {
		...
		const files= this._getAllBlogNamesSync();

		let length= 0;

		const blogs= files.map(file => {

			const content= fs.readFileSync(path.join(this.blogPath, file));

			length++;

			return {
				title: file.replace(/\.md$/gi, ''),
	...
	_getAllBlogNamesSync() {

		const fileList= fs.readdirSync(this.blogPath);

		return fileList.filter( file => /\.md$/gi.test(file) );
	}	
